...
constexpr bool hasRead(Perm p) {
    return (static_cast<uint8_t>(p & Perm::Read)) != 0;
}

constexpr bool hasWrite(Perm p) {
    return (static_cast<uint8_t>(p & Perm::Write)) != 0;
}
...
public:
    explicit VFSError(int code, const std::string& msg)
