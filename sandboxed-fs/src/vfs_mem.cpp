...
static uint64_t nowMs() {
    return static_cast<uint64_t>(std::time(nullptr)) * 1000;
}
...
void MemFSBackend::rmdir(const std::string& path) {
    unlink(path);
}
...
Stat MemFSBackend::lstat(const std::string& path) {
    return stat(path);
}
...
void MemFSBackend::rename(const std::string& oldPath, const std::string& newPath) {
    ...
    if (oldParts.empty()) oldParentPath = "/";
    ...
    if (newParts.empty()) newParentPath = "/";
...
int MemFSBackend::access(const std::string& path, int mode) {
    ...
    if ((mode & access_mode::R_OK) && !(stMode & 0400)) return EACCES;
    if ((mode & access_mode::W_OK) && !(stMode & 0200)) return EACCES;
    if ((mode & access_mode::X_OK) && !(stMode & 0100)) return EACCES;
