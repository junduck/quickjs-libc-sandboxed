...
VirtualFS::VirtualFS(std::vector<MountEntry> mounts)
        ...
        if (normPath == mount.prefix || normPath.starts_with(mount.prefix + "/")) {
            if (static_cast<uint8_t>(mount.perm & neededPerm) == 0) {
                throw VFSError(EACCES,
                               "VirtualFS: permission denied for '" + path +
                                   "' (needed " + (neededPerm == Perm::Read ? "read" : "write") +
                                   ", mount has " +
                                   (mount.perm == ReadWrite ? "readwrite" :
                                    mount.perm == Perm::Read ? "read" : "none") +
                                   ")");
            }
...
void VirtualFS::rename(const std::string& oldPath, const std::string& newPath) {
    ...
    if (oldBackend != newBackend) {
        throw VFSError(EXDEV,
                       "VirtualFS: cross-device rename not supported: '" +
                           oldPath + "' -> '" + newPath + "'");
    }
